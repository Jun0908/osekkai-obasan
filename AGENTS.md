...
- PUSHの推薦条件とMapの掲載条件を分離する。Mapは選択期間・地域で取得した全Eventを既定表示し、Connection Levelが低い、根拠未確認、満席、中止等のEventも状態と推薦対象外理由を明示して隠さない
- 単なる人気順や近さ順ではなく、本人の好み、隣接ジャンル、Calendar、移動、料金、過去Feedbackを同時に扱う
- Judge向け主経路は、条件を満たす少数の複数候補を推薦理由付きで提示する。件数を1件へ固定せず、候補がない場合に架空Eventを生成しない
